...
class Process1Service : Service() {
    private var connectivityManager: ConnectivityManager? = null
    private var network1: Network? = null
    ...
    override fun onCreate() {
        ...
        val request = NetworkRequest.Builder()
            .addTransportType(NetworkCapabilities.TRANSPORT_WIFI) // 绑定到 WIFI 网络
            .build()
        ...
        okHttpClient.newCall(Request.Builder().url(url).build())
            .enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    println("Process1Service=================onFailure: $e")
                }

                override fun onResponse(call: Call, response: Response) {
                    val string = response.body?.string()
                    println(("Process1Service=================onResponse: " + string))
                }
            })
    }
}